...
const AntDesignStyledDataGridPro = styled(DataGridPro)(({ theme }) => ({
    border: `1px solid ${theme.palette.mode === 'light' ? '#42413c' : '#f0f0f0'}`,
    color:
    ...
    '& .MuiDataGrid-columnsContainer': {
        backgroundColor: theme.palette.mode === 'light' ? '#fafafa' : '#000000',
    },
...
function SettingsPanel(props: GridToolbarContainerProps) {
    ...
    const handleSizeChange = React.useCallback((event:any) => {
    ...
    const handlePaginationChange = React.useCallback((event:any) => {
        setSelectedPaginationValue(event.target.value);
    }, []);

    const handleThemeChange = React.useCallback((event:any) => {
